...
let JwtStrategy = (() => {
    ...
    var JwtStrategy = class extends _classSuper {
        ...
        configService;
        usersRepository;
        logger = new common_1.Logger(JwtStrategy.name);
        ...
        async validate(payload) {
            ...
            if (!user || user.status === user_entity_1.UserStatus.SUSPENDED || user.status === user_entity_1.UserStatus.INACTIVE) {
                throw new common_1.UnauthorizedException('Compte utilisateur inactif ou suspendu');
            }
            return {
                id: user.id,
                email: user.email,
