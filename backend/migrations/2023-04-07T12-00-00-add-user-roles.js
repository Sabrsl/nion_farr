...
async function up(connection) {
    ...
    const result = await connection.collection('users').updateMany({ role: { $exists: false } }, { $set: { role: 'user' } });
...
async function down(connection) {
    ...
    const result = await connection.collection('users').updateMany({ role: 'user' }, { $unset: { role: 1 } });
