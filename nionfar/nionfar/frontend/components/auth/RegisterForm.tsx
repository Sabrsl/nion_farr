...
interface RegisterFormProps {
  defaultAccountType?: 'provider' | 'client';
}
...
const RegisterForm: React.FC<RegisterFormProps> = ({ defaultAccountType = 'client' }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    phoneNumber: '',
    termsAccepted: false,
    accountType: defaultAccountType as 'provider' | 'client',
  });
  ...
  const handleSelectAccountType = (type: 'provider' | 'client') => {
    setFormData(prev => ({
      ...prev,
      accountType: type
    }));
    ...
    <div className="space-y-5">
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <button
            type="button"
            onClick={() => handleSelectAccountType('provider')}
            className={`py-3 px-4 rounded-lg flex flex-col items-center justify-center border-2 transition-all hover:shadow-md ${
