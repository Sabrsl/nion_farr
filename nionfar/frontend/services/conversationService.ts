    ...
    try {
      ...
      if (process.env.NEXT_PUBLIC_API_MOCKING === 'enabled' || !this.apiUrl) {
        ...
        if (storedConversations) {
          ...
          const { conversations: mockConversations } = await import('../data/mockMessages');
        ...
        const enhancedConversations = await Promise.all(userConversations.map(async (conv: any) => {
          ...
          if (storedMessages) {
            ...
            const { messages: mockMessages } = await import('../data/mockMessages');
          ...
          if (typeof otherParticipant === 'string') {
            ...
            const { conversations: mockUsers } = await import('../data/mockMessages');
