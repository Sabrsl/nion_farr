...
import { categories, freelancerServices } from '../../../../data/mockData';
...
const EditServicePage: NextPage = () => {
  ...
  useEffect(() => {
    if (id) {
      ...
      // Simulate an API call to fetch the service
      const serviceData = freelancerServices.find(s => s.id === id);
      
      if (serviceData) {
        setService(serviceData);
        
        // Convert imageUrls to strings for display
        ...
        const formData = {
          ...
          hasPackages: false, // Assuming no packages for now
          packageOptions: {
            basic: {
              title: 'Basique',
              description: 'Option de base pour votre service',
        ...
        // Service not found
        setSaveStatus('error');
        // Redirect back to services list after a delay
        setTimeout(() => {
