...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // En production, vous feriez une requête à l'API Orange Money pour vérifier le statut
    /* 
    const statusResponse = await fetch(`https://api.orange.com/orange-money-webpay/senegal/v1/transactionstatus`, {
      method: 'POST',
      headers: {
    ...
    */
    ...
    console.log(`[ORANGE MONEY PAYMENT] Vérification du statut pour: ${referenceId}`);
    ...
    console.error('[ORANGE MONEY STATUS API] Erreur:', error);
